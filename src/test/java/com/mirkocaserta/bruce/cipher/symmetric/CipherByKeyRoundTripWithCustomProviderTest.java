...
class CipherByKeyRoundTripWithCustomProviderTest {
    ...
    void roundTrip() {
        ...
        CipherByKey encrypter = cipherBuilder().keyAlgorithm("DESede").algorithm("DESede/CBC/PKCS5Padding").provider("BC").mode(ENCRYPT).buildSymmetricRawByKey();
        CipherByKey decrypter = cipherBuilder().keyAlgorithm("DESede").algorithm("DESede/CBC/PKCS5Padding").provider("BC").mode(DECRYPT).buildSymmetricRawByKey();
