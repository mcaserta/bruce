...
class CipherRoundTripWithCustomProviderTest {
    ...
    void roundTrip() {
        ...
        SymmetricCipher encrypter = cipherBuilder().key(key).keyAlgorithm("DESede").algorithm("DESede/CBC/PKCS5Padding").provider("BC").mode(ENCRYPT).buildSymmetricRaw();
        SymmetricCipher decrypter = cipherBuilder().key(key).keyAlgorithm("DESede").algorithm("DESede/CBC/PKCS5Padding").provider("BC").mode(DECRYPT).buildSymmetricRaw();
