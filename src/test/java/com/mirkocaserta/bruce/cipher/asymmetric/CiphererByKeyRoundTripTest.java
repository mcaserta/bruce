...
class CiphererByKeyRoundTripTest {
    ...
    void roundTrip() {
        ...
        CiphererByKey cipherer = cipherer(keys, "RSA");

        // Alice writes to Bob
        byte[] aliceMsg01 = "Hello".getBytes(UTF_8);
        byte[] aliceMsg01Encrypted = cipherer.encrypt("bob-public", ENCRYPT, aliceMsg01);
        assertNotNull(aliceMsg01Encrypted);

        // Bob decrypts Alice's message
        byte[] aliceMsg01Decrypted = cipherer.encrypt("bob-private", DECRYPT, aliceMsg01Encrypted);
        ...
        byte[] bobMsg01Encrypted = cipherer.encrypt("alice-public", ENCRYPT, bobMsg01);
        assertNotNull(bobMsg01Encrypted);

        // Alice decrypts Bob's message
        byte[] bobMsg01Decrypted = cipherer.encrypt("alice-private", DECRYPT, bobMsg01Encrypted);
