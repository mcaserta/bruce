...
class EncodingSignerAndVerifierUrlTest extends EncodingSignerAndVerifierCommonTest {
    ...
    void signerAndVerifiersWithWrongParamsShouldFail() {
        final KeyStore keystore = keystore("classpath:/keystore.p12", "password", "PKCS12");
        assertThrows(BruceException.class, () -> Bruce.signer(privateKey(keystore, "sgiao belo", "password"), Bruce.Encoding.URL));
        assertThrows(BruceException.class, () -> Bruce.signer(privateKey(keystore, "test", "sgiao belo"), Bruce.Encoding.URL));
        assertThrows(BruceException.class, () -> Bruce.verifier(publicKey(keystore, "sgiao belo"), Bruce.Encoding.URL));
    }
