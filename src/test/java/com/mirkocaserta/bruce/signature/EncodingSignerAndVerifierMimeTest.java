...
class EncodingSignerAndVerifierMimeTest extends EncodingSignerAndVerifierCommonTest {
    ...
    protected EncodingSigner getSigner() {
        ...
        return signerBuilder().key(privateKey(keystore, "test", "password".toCharArray())).algorithm("SHA512withRSA").encoding(Bruce.Encoding.MIME).build();
    }

    @Override
    protected EncodingVerifier getVerifier() {
        final KeyStore keystore = keystore("classpath:/keystore.p12", "password".toCharArray(), "PKCS12");
        return verifierBuilder().key(publicKey(keystore, "test")).algorithm("SHA512withRSA").encoding(Bruce.Encoding.MIME).build();
    }
