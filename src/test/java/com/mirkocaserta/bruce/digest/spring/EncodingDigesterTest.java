...
class EncodingDigesterTest {
    ...
    public static class Cfg {
        ...
        public EncodingDigester sha1hex() {
            return Crypt.digester("SHA1", Crypt.Encoding.HEX);
        }

        @Bean
        public EncodingDigester sha1base64() {
            return Crypt.digester("SHA1", Crypt.Encoding.BASE64);
        }

        @Bean
        public EncodingDigester sha1url() {
            return Crypt.digester("SHA1", Crypt.Encoding.URL);
        }

        @Bean
        public EncodingDigester sha1mime() {
            return Crypt.digester("SHA1", Crypt.Encoding.MIME);
        }

        @Bean
        public EncodingDigester md5hex() {
            return Crypt.digester("MD5", Crypt.Encoding.HEX);
        }

        @Bean
        public EncodingDigester md5base64() {
            return Crypt.digester("MD5", Crypt.Encoding.BASE64);
        }

        @Bean
        public EncodingDigester md5url() {
            return Crypt.digester("MD5", Crypt.Encoding.URL);
        }

        @Bean
        public EncodingDigester md5mime() {
            return Crypt.digester("MD5", Crypt.Encoding.MIME);
        }
    }
