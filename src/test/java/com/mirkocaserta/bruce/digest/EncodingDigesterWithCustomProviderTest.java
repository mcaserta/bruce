...
class EncodingDigesterWithCustomProviderTest {
    ...
    void sha1Hex() {
        EncodingDigester digester = digestBuilder().algorithm("SHA1").provider("BC").encoding(Bruce.Encoding.HEX).build();
    ...
    void sha1Base64() {
        EncodingDigester digester = digestBuilder().algorithm("SHA1").provider("BC").encoding(Bruce.Encoding.BASE64).build();
    ...
    void sha1Url() {
        EncodingDigester digester = digestBuilder().algorithm("SHA1").provider("BC").encoding(Bruce.Encoding.URL).build();
    ...
    void sha1MIME() {
        EncodingDigester digester = digestBuilder().algorithm("SHA1").provider("BC").encoding(Bruce.Encoding.MIME).build();
        assertEquals("b5ua881ui4pzws3O03/p9ZIm4n0=", digester.digest("message"), "1st sha1");
        assertEquals("2jmj7l5rSw0yVb/vlWAYkK/YBwk=", digester.digest(""), "2nd sha1");
    }
    ...
    void md5Hex() {
        EncodingDigester digester = digestBuilder().algorithm("MD5").provider("BC").encoding(Bruce.Encoding.HEX).build();
    ...
    void md5Base64() {
        EncodingDigester digester = digestBuilder().algorithm("MD5").provider("BC").encoding(Bruce.Encoding.BASE64).build();
    ...
    void md5Url() {
        EncodingDigester digester = digestBuilder().algorithm("MD5").provider("BC").encoding(Bruce.Encoding.URL).build();
        assertEquals("eOcxAn2P1Q7WQjQLfJpjsw==", digester.digest("message"), "1st md5");
        assertEquals("1B2M2Y8AsgTpgAmY7PhCfg==", digester.digest(""), "2nd md5");
    }
    ...
    void md5MIME() {
        EncodingDigester digester = digestBuilder().algorithm("MD5").provider("BC").encoding(Bruce.Encoding.MIME).build();
        assertEquals("eOcxAn2P1Q7WQjQLfJpjsw==", digester.digest("message"), "1st md5");
        assertEquals("1B2M2Y8AsgTpgAmY7PhCfg==", digester.digest(""), "2nd md5");
    }
    ...
    void invalidAlgorithm1() {
        Assertions.assertThrows(
                ...
                () -> digestBuilder().algorithm("foo").provider("BC").encoding(Bruce.Encoding.HEX).build(),
