...
class DigesterTest {
  ...
  void invalidAlgorithm1() {
    Assertions.assertThrows(BruceException.class, () -> digester("foo"), "No such algorithm: foo");
  }
  ...
  void invalidAlgorithm2() {
    assertThrows(BruceException.class, () -> digester("foo", "bar"), "No such algorithm: foo");
  }

  @Test
  @DisplayName("Digester for an invalid provider should throw a DigesterException")
  void invalidProvider() {
    assertThrows(BruceException.class, () -> digester("SHA1", "foo"), "No such provider: foo");
  }
  ...
  void invalidEncoder() {
    assertThrows(
        BruceException.class, () -> digester("SHA1", "SUN", null), "No such encoding: null");
  }
}
