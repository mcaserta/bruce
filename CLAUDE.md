...
Cryptography API (JCA). It provides simplified interfaces for common
...
All cryptographic operations are accessed through static methods on the `Bruce`
class.
...
- `Bruce.java` - Central facade providing static factory methods for all
...
**Encoding Support:** Operations that work with text support multiple encodings
via the `Encoding` enum (HEX, BASE64, URL, MIME).

**Provider Support:** All operations support pluggable cryptographic providers
(defaults to system, but can specify Bouncy Castle "BC" or others).
...
The project uses Java 9+ modules defined in `module-info.java`. Exports all
public API packages but keeps internal implementation details private.
